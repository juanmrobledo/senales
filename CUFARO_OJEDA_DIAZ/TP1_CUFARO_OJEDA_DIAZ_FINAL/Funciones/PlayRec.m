...
%     Funci�n para grabar y reproducir un sine sweep en tiempo real.
...
%     fm = Frecuencia de muestreo
    ...
    fileReader = dsp.AudioFileReader('Example_SineSweep.wav');
    fileWriter = dsp.AudioFileWriter('MicRecorded.wav', 'SampleRate', fm);
    aPR = audioPlayerRecorder('SampleRate',fm);
    release(aPR)

    try
        while ~isDone(fileReader)          %Loop de adquisicion de se�al
